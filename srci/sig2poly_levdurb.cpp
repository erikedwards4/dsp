...
descr += "and output Y holds the linear prediction (LP) polynomials.\n";
...
descr += "\n";
descr += "If dim==0, then Y has size (P+1) x C.\n";
descr += "If dim==1, then Y has size R x (P+1).\n";
descr += "\n";
descr += "Examples:\n";
descr += "$ sig2poly_levdurb -p3 X -o Y -o V \n";
descr += "$ sig2poly_levdurb -d1 -p5 X -o Y -o V \n";
descr += "$ cat X | sig2poly_levdurb -p7 -m > Y \n";
...
o1.F = o2.F = i1.F;
o1.T = o2.T = i1.T;
o1.R = (dim==0u) ? uint32_t(P+1) : i1.R;
o1.C = (dim==1u) ? uint32_t(P+1) : i1.C;
o2.R = (dim==0u) ? 1u : i1.R;
o2.C = (dim==1u) ? 1u : i1.C;
o1.S = o2.S = i1.S;
o1.H = o2.H = i1.H;
...
if (i1.T==1u)
{
    ...
    if (codee::sig2poly_levdurb_s(Y,V,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,P,m0)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
