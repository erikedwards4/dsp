...
descr += "\n";
descr += "The output (Y) is real-valued with length ndft along dim. \n";
descr += "\n";
descr += "For complex X, Y is complex and consists of the DFT of the \n";
descr += "real and imag parts separately (like Octave convention).\n";
descr += "\n";
descr += "Examples:\n";
descr += "$ dft.cblas -n256 X -o Y \n";
descr += "$ dft.cblas -n256 -d1 X > Y \n";
descr += "$ cat X | dft.cblas -n256 > Y \n";
...
if (i1.T==1u)
{
    ...
    if (codee::dft_cblas_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
...
//     try { ifs1.read(reinterpret_cast<char*>(X),i1.nbytes()); }
//     catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem reading input file (X)" << endl; return 1; }
//     if (codee::dft_cblas_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
//     { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
//     if (wo1)
//     {
//         try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
