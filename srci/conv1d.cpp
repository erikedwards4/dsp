...
descr += "$ conv1d -d1 -c-2 -w500 X1 X2 > Y \n";
descr += "$ cat X2 | conv1d -c2 -w490 -i2 -s3 X1 > Y \n";
...
if (i1.T==1u)
{
    ...
    if (codee::conv1d_s(Y,X,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),W,c0,stp,dil,dim))
...
else if (i1.T==101u)
{
    ...
    if (codee::conv1d_c(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),W,c0,stp,dil,dim))
    { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
