...
//Description
string descr;
descr += "Gets autocovariance of each column or row of X using FFT method.\n";
descr += "The means are NOT subtracted before taking ifft(|fft|.^2).\n";
...
descr += "$ sig2ac_fft -d1 -l127 X > Y \n";
descr += "$ cat X | sig2ac_fft -l127 > Y \n";
...
if (i1.T==1u)
{
    ...
    if (codee::sig2ac_fft_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,L,u)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
...
else if (i1.T==101u)
{
    ...
    if (codee::sig2ac_fft_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,L,u)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
