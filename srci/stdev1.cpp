...
struct arg_lit    *a_b = arg_litn("b","biased",0,1,"use biased (N) denominator [default=unbiased]");
...
if (i1.T==1u)
{
    ...
    if (codee::stdev1_s(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,b)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
...
else if (i1.T==101u)
{
    ...
    if (codee::stdev1_c(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,b)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
