...
//Description
string descr;
descr += "Gets power spectral density (PSD) from polynomial coeffs along rows or cols of X.\n";
...
descr += "\n";
descr += "Examples:\n";
descr += "$ poly2psd X V W -o Y \n";
descr += "$ poly2psd X V W > Y \n";
descr += "$ cat W | poly2psd X V > Y \n";
...
if (dim==0u && i1.C!=i2.N()) { cerr << progstr+": " << __LINE__ << errstr << "inputs 1 and 2 must be size compatible" << endl; return 1; }
if (dim==1u && i1.R!=i2.N()) { cerr << progstr+": " << __LINE__ << errstr << "inputs 1 and 2 must be size compatible" << endl; return 1; }

//Set output header info
o1.F = i1.F; o1.T = (i1.isreal()) ? i1.T : i1.T-100u;
...
if (i1.T==1u)
{
    ...
    if (codee::poly2psd_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),V,W,int(i3.N()),dim))
...
else if (i1.T==101u)
{
    ...
    if (codee::poly2psd_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),V,W,int(i3.N()),dim))
    { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
