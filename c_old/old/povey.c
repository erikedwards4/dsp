...
int povey_s (float *X, const size_t L, const char normalize);
int povey_d (double *X, const size_t L, const char normalize);
int povey_c (float *X, const size_t L, const char normalize);
int povey_z (double *X, const size_t L, const char normalize);
...
int povey_s (float *X, const size_t L, const char normalize)
{
    ...
    while (l<L/2) { X[l] = powf(0.5f-0.5f*cosf(p*l),0.85f); l++; }
    if (L%2) { X[l] = 1.0f; l++; }
...
int povey_d (double *X, const size_t L, const char normalize)
{
    ...
    while (l<L/2) { X[l] = pow(0.5-0.5*cos(p*l),0.85); l++; }
    if (L%2) { X[l] = 1.0; l++; }
    while (l<L) { X[l] = X[L-l-1]; l++; }
...
int povey_c (float *X, const size_t L, const char normalize)
{
    ...
    while (l<L/2) { X[2*l] = powf(0.5f-0.5f*cosf(p*l),0.85f); l++; }
    if (L%2) { X[2*l] = 1.0f; l++; }
...
int povey_z (double *X, const size_t L, const char normalize)
{
    ...
    while (l<L/2) { X[2*l] = pow(0.5-0.5*cos(p*l),0.85); l++; }
    if (L%2) { X[2*l] = 1.0; l++; }
    while (l<L) { X[2*l] = X[2*(L-l-1)]; l++; }
