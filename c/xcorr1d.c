//1D cross-correlation by X2 of each vector in X1 along dim.
...
int xcorr1d_s (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const size_t pad, const size_t str, const size_t dil, const size_t dim)
{
    ...
    //if (c0>=(int)L1) { fprintf(stderr,"error in xcorr1d_s: c0 (center samp of 1st frame) must be < L1 (length of vecs in X1)\n"); return 1; }

    const int N2 = (int)(dil*(L2-1u)) + 1;      //full length of X2 including dil
    ...
    //Set W (L_out)
    W = 1 + ((int)L1 + 2*(int)pad - (int)(dil*(L2-1u)) - 1) / str;
