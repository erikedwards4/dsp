...
//FIR filtering is similar, except fir_fft is causal and conv_fft is non-causal.

//This version uses the FFT for computation.
//It is faster than conv only if L1+L2 is a power-of-2, or just less than a power-of-2.
//Even then, it is slower including the FFT set-up;
//so would only make sense when one could do the FFT the set-up once and conv repeatedly.
...
int conv_fft_s (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const char shape[], const size_t dim)
{
    ...
    else
    {
        ...
        //if (nfft%2u) { fprintf(stderr,"error in conv_fft_s: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in conv_fft_s: problem creating fftw plan"); return 1; }
        fftwf_plan fplan2 = fftwf_plan_dft_r2c_1d((int)nfft,X2f,(fftwf_complex *)Y2f,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in conv_fft_s: problem creating fftw plan"); return 1; }
...
int conv_fft_d (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const char shape[], const size_t dim)
{
    ...
    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L1<1u) { fprintf(stderr,"error in conv_fft_d: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv_fft_d: L2 (length X2) must be positive\n"); return 1; }
    ...
    {
        fprintf(stderr,"error in conv_fft_d: shape string must be 'full', 'same' or 'valid'\n"); return 1;
    }
    ...
    else
    {
        ...
        while (nfft<L1+L2) { nfft *= 2u; }
        const size_t F = nfft/2u + 1u;
        //if (nfft%2u) { fprintf(stderr,"error in conv_fft_d: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in conv_fft_d: problem creating fftw plan"); return 1; }
        fftw_plan fplan2 = fftw_plan_dft_r2c_1d((int)nfft,X2f,(fftw_complex *)Y2f,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in conv_fft_d: problem creating fftw plan"); return 1; }
...
{
    if (dim>3u) { fprintf(stderr,"error in conv_fft_c: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L1<1u) { fprintf(stderr,"error in conv_fft_c: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv_fft_c: L2 (length X2) must be positive\n"); return 1; }
    ...
    {
        fprintf(stderr,"error in conv_fft_c: shape string must be 'full', 'same' or 'valid'\n"); return 1;
    }
    ...
    else
    {
        ...
        size_t nfft = 1u;
        while (nfft<L1+L2) { nfft *= 2u; }
        //if (nfft%2u) { fprintf(stderr,"error in conv_fft_c: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in conv_fft_c: problem creating fftw plan"); return 1; }
        fftwf_plan fplan2 = fftwf_plan_dft_1d((int)nfft,(fftwf_complex *)X2f,(fftwf_complex *)Y2f,FFTW_FORWARD,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in conv_fft_c: problem creating fftw plan"); return 1; }
        ...
        //Get Y2f (scaled FFT of X2f)
        for (size_t n=L2; n>0u; --n, ++X2, ++X2f) { *X2f = *X2; *++X2f = *++X2; }
...
int conv_fft_z (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const char shape[], const size_t dim)
{
    ...
    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L1<1u) { fprintf(stderr,"error in conv_fft_z: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv_fft_z: L2 (length X2) must be positive\n"); return 1; }
    ...
    {
        fprintf(stderr,"error in conv_fft_z: shape string must be 'full', 'same' or 'valid'\n"); return 1;
    }
    ...
    else
    {
        ...
        size_t nfft = 1u;
        while (nfft<L1+L2) { nfft *= 2u; }
        //if (nfft%2u) { fprintf(stderr,"error in conv_fft_z: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in conv_fft_z: problem creating fftw plan"); return 1; }
        fftw_plan fplan2 = fftw_plan_dft_1d((int)nfft,(fftw_complex *)X2f,(fftw_complex *)Y2f,FFTW_FORWARD,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in conv_fft_z: problem creating fftw plan"); return 1; }
