...
int idst_fftw_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    if (dim>3u) { fprintf(stderr,"error in idst_fftw_s: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<L) { fprintf(stderr,"error in idst_fftw_s: ndst must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!plan) { fprintf(stderr,"error in idst_fftw_s: problem creating fftw plan"); return 1; }
...
int idst_fftw_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    if (dim>3u) { fprintf(stderr,"error in idst_fftw_d: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<L) { fprintf(stderr,"error in idst_fftw_d: ndst must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!plan) { fprintf(stderr,"error in idst_fftw_d: problem creating fftw plan"); return 1; }
...
int idst_fftw_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    if (dim>3u) { fprintf(stderr,"error in idst_fftw_c: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<L) { fprintf(stderr,"error in idst_fftw_c: ndst must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!rplan) { fprintf(stderr,"error in idst_fftw_c: problem creating fftw plan"); return 1; }
        fftwf_plan iplan = fftwf_plan_r2r_1d((int)ndst,X1i,Y1i,FFTW_RODFT00,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in idst_fftw_c: problem creating fftw plan"); return 1; }
...
int idst_fftw_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    if (dim>3u) { fprintf(stderr,"error in idst_fftw_z: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<L) { fprintf(stderr,"error in idst_fftw_z: ndst must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!rplan) { fprintf(stderr,"error in idst_fftw_z: problem creating fftw plan"); return 1; }
        fftw_plan iplan = fftw_plan_r2r_1d((int)ndst,X1i,Y1i,FFTW_RODFT00,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in idst_fftw_z: problem creating fftw plan"); return 1; }
