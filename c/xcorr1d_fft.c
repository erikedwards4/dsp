...
//This gives identical output to xcorr1d, but uses FFT internally.
//See xcorr1d for more info.

//I could not find any condition where this is faster than xcorr1d,
//even when using near power-of-2 N1+N2.
//xcorr1d is also more numerically accurate.
...
int xcorr1d_fft_s (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const size_t pad, const size_t str, const size_t dil, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in xcorr1d_fft_s: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in xcorr1d_fft_s: str (stride) must be positive\n"); return 1; }
    if (dil<1u) { fprintf(stderr,"error in xcorr1d_fft_s: dil (dilation) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in xcorr1d_fft_s: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in xcorr1d_fft_s: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in xcorr1d_fft_s: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L1+2u*pad<=dil*(L2-1u)) { fprintf(stderr,"error in xcorr1d_fft_s: L1+2*pad must be > dil*(L2-1)\n"); return 1; }
    ...
    else
    {
        ...
        //if (nfft%2u) { fprintf(stderr,"error in xcorr1d_fft_s: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in xcorr1d_fft_s: problem creating fftw plan"); return 1; }
        fftwf_plan fplan2 = fftwf_plan_dft_r2c_1d((int)nfft,X2f,(fftwf_complex *)Y2f,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in xcorr1d_fft_s: problem creating fftw plan"); return 1; }
        ...
        if (!iplan) { fprintf(stderr,"error in xcorr1d_fft_s: problem creating fftw plan"); return 1; }
        for (size_t n=2u*nfft; n>0u; --n, ++Y1f) { *Y1f = 0.0f; }
        Y1f -= 2u*nfft;

        //Get Y2f (scaled FFT of reversed X2f with dilation holes)
        X2 += L2;
        for (size_t n=L2; n>0u; --n, ++X2f)
        {
            *X2f = *--X2;
...
int xcorr1d_fft_d (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const size_t pad, const size_t str, const size_t dil, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in xcorr1d_fft_d: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in xcorr1d_fft_d: str (stride) must be positive\n"); return 1; }
    if (dil<1u) { fprintf(stderr,"error in xcorr1d_fft_d: dil (dilation) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in xcorr1d_fft_d: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in xcorr1d_fft_d: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in xcorr1d_fft_d: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L1+2u*pad<=dil*(L2-1u)) { fprintf(stderr,"error in xcorr1d_fft_d: L1+2*pad must be > dil*(L2-1)\n"); return 1; }
    ...
    else
    {
        ...
        if (!fplan1) { fprintf(stderr,"error in xcorr1d_fft_d: problem creating fftw plan"); return 1; }
        fftw_plan fplan2 = fftw_plan_dft_r2c_1d((int)nfft,X2f,(fftw_complex *)Y2f,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in xcorr1d_fft_d: problem creating fftw plan"); return 1; }
        ...
        if (!iplan) { fprintf(stderr,"error in xcorr1d_fft_d: problem creating fftw plan"); return 1; }
...
int xcorr1d_fft_c (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const size_t pad, const size_t str, const size_t dil, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in xcorr1d_fft_c: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in xcorr1d_fft_c: str (stride) must be positive\n"); return 1; }
    if (dil<1u) { fprintf(stderr,"error in xcorr1d_fft_c: dil (dilation) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in xcorr1d_fft_c: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in xcorr1d_fft_c: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in xcorr1d_fft_c: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L1+2u*pad<=dil*(L2-1u)) { fprintf(stderr,"error in xcorr1d_fft_c: L1+2*pad must be > dil*(L2-1)\n"); return 1; }
    ...
    else
    {
        ...
        size_t nfft = 1u;
        while ((int)nfft<N1+N2) { nfft *= 2u; }
        //if (nfft%2u) { fprintf(stderr,"error in xcorr1d_fft_c: nfft must be even"); return 1; }
        ...
        if (!fplan1) { fprintf(stderr,"error in xcorr1d_fft_c: problem creating fftw plan"); return 1; }
        fftwf_plan fplan2 = fftwf_plan_dft_1d((int)nfft,(fftwf_complex *)X2f,(fftwf_complex *)Y2f,FFTW_FORWARD,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in xcorr1d_fft_c: problem creating fftw plan"); return 1; }
        ...
        Z1f = (float *)fftwf_malloc(2u*nfft*sizeof(float));
        fftwf_plan iplan = fftwf_plan_dft_1d((int)nfft,(fftwf_complex *)Y1f,(fftwf_complex *)Z1f,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in xcorr1d_fft_c: problem creating fftw plan"); return 1; }
        for (size_t n=2u*nfft; n>0u; --n, ++Y1f) { *Y1f = 0.0f; }
        Y1f -= 2u*nfft;
...
int xcorr1d_fft_z (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const size_t pad, const size_t str, const size_t dil, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in xcorr1d_fft_z: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in xcorr1d_fft_z: str (stride) must be positive\n"); return 1; }
    if (dil<1u) { fprintf(stderr,"error in xcorr1d_fft_z: dil (dilation) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in xcorr1d_fft_z: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in xcorr1d_fft_z: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in xcorr1d_fft_z: L1 (length of vecs in X1) must be positive\n"); return 1; }
    if (L1+2u*pad<=dil*(L2-1u)) { fprintf(stderr,"error in xcorr1d_fft_z: L1+2*pad must be > dil*(L2-1)\n"); return 1; }
    ...
    else
    {
        ...
        if (!fplan1) { fprintf(stderr,"error in xcorr1d_fft_z: problem creating fftw plan"); return 1; }
        fftw_plan fplan2 = fftw_plan_dft_1d((int)nfft,(fftw_complex *)X2f,(fftw_complex *)Y2f,FFTW_FORWARD,FFTW_ESTIMATE);
        if (!fplan2) { fprintf(stderr,"error in xcorr1d_fft_z: problem creating fftw plan"); return 1; }
        ...
        Z1f = (double *)fftw_malloc(2u*nfft*sizeof(double));
        fftw_plan iplan = fftw_plan_dft_1d((int)nfft,(fftw_complex *)Y1f,(fftw_complex *)Z1f,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in xcorr1d_fft_z: problem creating fftw plan"); return 1; }
        for (size_t n=2u*nfft; n>0u; --n, ++Y1f) { *Y1f = 0.0; }
        Y1f -= 2u*nfft;
