...
int dst_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    ...
    if (ndst<Lx) { fprintf(stderr,"error in dst_s: ndst must be >= Lx (length of vecs in X)\n"); return 1; }
    ...
    else
    {
        ...
        if (!DST) { fprintf(stderr,"error in dst_s: problem with aligned_alloc. "); perror("aligned_alloc"); return 1; }
...
int dst_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    ...
    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<Lx) { fprintf(stderr,"error in dst_d: ndst must be >= Lx (length of vecs in X)\n"); return 1; }
    ...
    else
    {
        ...
        if (!DST) { fprintf(stderr,"error in dst_d: problem with aligned_alloc. "); perror("aligned_alloc"); return 1; }
        for (size_t n=0u; n<ndst; ++n)
        {
            for (size_t l=0u; l<Lx; ++l, ++DST)
            {
...
int dst_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    ...
    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<Lx) { fprintf(stderr,"error in dst_c: ndst must be >= Lx (length of vecs in X)\n"); return 1; }
    ...
    else
    {
        ...
        float *DST, smr, smi;
        DST = (float *)aligned_alloc(sizeof(float),LN*sizeof(float));
        if (!DST) { fprintf(stderr,"error in dst_c: problem with aligned_alloc. "); perror("aligned_alloc"); return 1; }
        for (size_t n=0u; n<ndst; ++n)
        {
            for (size_t l=0u; l<Lx; ++l, ++DST)
            {
...
int dst_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t ndst, const int sc)
{
    ...
    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (ndst<Lx) { fprintf(stderr,"error in dst_z: ndst must be >= Lx (length of vecs in X)\n"); return 1; }
    ...
    else
    {
        ...
        double *DST, smr, smi;
        DST = (double *)aligned_alloc(sizeof(double),LN*sizeof(double));
        if (!DST) { fprintf(stderr,"error in dst_z: problem with aligned_alloc. "); perror("aligned_alloc"); return 1; }
        for (size_t n=0u; n<ndst; ++n)
        {
            for (size_t l=0u; l<Lx; ++l, ++DST)
            {
