...
//Y is the imaginary part of the analytic signal (AS).
//This is unlike Octave's hilbert, which outputs the AS.
...
int hilbert_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t nfft)
{
    ...
    {
        for (size_t n=0u; n<N; ++n, ++X, ++Y) { *Y = 0.0f; }
    }
    else
    {
        ...
        if (L==N)
        {
            ...
            *Y1++ /= (float)nfft; ++Y1;
            ...
            fftwf_execute(iplan);
            for (size_t l=0u; l<L; ++l, ++Z1, ++Y) { *Y = *++Z1; }
            Z1 -= 2u*L;
        }
        else
        {
                ...
                for (size_t g=G; g>0u; --g, X+=B*(L-1u), Y+=B*(L-1u))
                {
                    for (size_t b=B; b>0u; --b, X-=K*L-1u, Z1-=2u*L, Y-=K*L-1u)
                    {
                        ...
                        *Y1++ /= (float)nfft; ++Y1;
                        ...
                        Y1 -= nfft + 1u;
                        fftwf_execute(iplan);
                        for (size_t l=0u; l<L; ++l, ++Z1, Y+=K) { *Y = *++Z1; }
                    }
                }
            }
        }
