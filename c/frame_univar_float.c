...
//To use those conventions, and integer-only stp (frame-shift), see frame_univar.c.
...
int frame_univar_float_d (double *Y, const double *X, const size_t N, const size_t W, const size_t L, const double c0, const double stp)
{
    if (L>=N) { fprintf(stderr,"error in frame_univar_float_d: L must be < N (length X)\n"); return 1; }
    if (L<1u) { fprintf(stderr,"error in frame_univar_float_d: L must be positive\n"); return 1; }
    if (N<1u) { fprintf(stderr,"error in frame_univar_float_d: N (length X) must be positive\n"); return 1; }
    if (c0>N-1u) { fprintf(stderr,"error in frame_univar_float_d: c0 (center samp of 1st frame) must be < N (length X)\n"); return 1; }
    if (stp<DBL_EPSILON) { fprintf(stderr,"error in frame_univar_float_d: stp (step size) must be positive\n"); return 1; }
    ...
    else
    {
        ...
        size_t w = 0u;                              //current frame
        float cc = c0;                              //current exact center-samp
        int cs = (int)round(cc);                   //current rounded center-samp
        int ss, es = cs + (int)Lpost;               //current rounded start-samp, end-samp
        int prev_cs;                                //previous rounded center-samp
        ...
        {
            for (size_t l=0u; l<N+Lpre-(size_t)cs; ++l, ++X, ++Y) { *Y = *X; }
            for (size_t l=N+Lpre-(size_t)cs; l<L; ++l, ++Y) { *Y = 0.0; }
            X -= N + Lpre - (size_t)cs;
            ss = cs - (int)Lpre;
            ++w; cc += stp; cs = (int)round(cc);
            X += cs - prev_cs; prev_cs = cs;
...
int frame_univar_float_c (float *Y, const float *X, const size_t N, const size_t W, const size_t L, const float c0, const float stp)
{
    if (L>=N) { fprintf(stderr,"error in frame_univar_float_c: L must be < N (length X)\n"); return 1; }
    if (L<1u) { fprintf(stderr,"error in frame_univar_float_c: L must be positive\n"); return 1; }
    if (N<1u) { fprintf(stderr,"error in frame_univar_float_c: N (length X) must be positive\n"); return 1; }
    if (c0>N-1u) { fprintf(stderr,"error in frame_univar_float_c: c0 (center samp of 1st frame) must be < N (length X)\n"); return 1; }
    if (stp<FLT_EPSILON) { fprintf(stderr,"error in frame_univar_float_c: stp (step size) must be positive\n"); return 1; }
    ...
    else
    {
        ...
        while (ss<(int)N && w<W)
        {
            for (size_t l=0u; l<N+Lpre-(size_t)cs; ++l, ++X, ++Y) { *Y = *X; *++Y = *++X; }
            for (size_t l=N+Lpre-(size_t)cs; l<L; ++l, ++Y) { *Y = 0.0f; *++Y = 0.0f; }
            X -= 2u*(N+Lpre-(size_t)cs);
            ss = cs - (int)Lpre;
            ++w; cc += stp; cs = (int)roundf(cc);
            X += 2u*(cs-prev_cs); prev_cs = cs;
        }
...
int frame_univar_float_z (double *Y, const double *X, const size_t N, const size_t W, const size_t L, const double c0, const double stp)
{
    if (L>=N) { fprintf(stderr,"error in frame_univar_float_z: L must be < N (length X)\n"); return 1; }
    if (L<1u) { fprintf(stderr,"error in frame_univar_float_z: L must be positive\n"); return 1; }
    if (N<1u) { fprintf(stderr,"error in frame_univar_float_z: N (length X) must be positive\n"); return 1; }
    if (c0>N-1u) { fprintf(stderr,"error in frame_univar_float_z: c0 (center samp of 1st frame) must be < N (length X)\n"); return 1; }
    if (stp<DBL_EPSILON) { fprintf(stderr,"error in frame_univar_float_z: stp (step size) must be positive\n"); return 1; }
    ...
    else
    {
        ...
        {
            for (size_t l=0u; l<N+Lpre-(size_t)cs; ++l, ++X, ++Y) { *Y = *X; *++Y = *++X; }
            for (size_t l=N+Lpre-(size_t)cs; l<L; ++l, ++Y) { *Y = 0.0; *++Y = 0.0; }
            X -= 2u*(N+Lpre-(size_t)cs);
            ss = cs - (int)Lpre;
            ++w; cc += stp; cs = (int)round(cc);
            X += 2u*(cs-prev_cs); prev_cs = cs;
