...
int hann_s (float *Y, const size_t L, const size_t norm)
{
    if (norm>3) { fprintf(stderr,"error in hann_s: norm must be in {0,1,2,3}\n"); return 1; }

    const float p = (float)(2.0*M_PI/(L-1));
    
    for (size_t l=0; l<L/2; ++l, ++Y) { *Y = 0.5f - 0.5f*cosf(l*p); }
    if (L%2) { *Y++ = 1.0f; }
...
int hann_d (double *Y, const size_t L, const size_t norm)
{
    if (norm>3) { fprintf(stderr,"error in hann_d: norm must be in {0,1,2,3}\n"); return 1; }

    const double p = 2.0*M_PI/(L-1.0);
    
    for (size_t l=0; l<L/2; ++l, ++Y) { *Y = 0.5 - 0.5*cos(l*p); }
    if (L%2) { *Y++ = 1.0; }
    for (size_t l=0; l<L/2; ++l, ++Y) { *Y = *(Y-2*l-1-L%2); }
...
int hann_c (float *Y, const size_t L, const size_t norm)
{
    if (norm>3) { fprintf(stderr,"error in hann_c: norm must be in {0,1,2,3}\n"); return 1; }

    const float p = (float)(2.0*M_PI/(L-1));
    
    for (size_t l=0; l<L/2; ++l, ++Y) { *Y = 0.5f - 0.5f*cosf(l*p); *++Y = 0.0f; }
...
int hann_z (double *Y, const size_t L, const size_t norm)
{
    if (norm>3) { fprintf(stderr,"error in hann_z: norm must be in {0,1,2,3}\n"); return 1; }

    const double p = 2.0*M_PI/(L-1.0);
    
    for (size_t l=0; l<L/2; ++l, ++Y) { *Y = 0.5 - 0.5*cos(l*p); *++Y = 0.0; }
    if (L%2) { *Y++ = 1.0; *Y++ = 0.0; }
