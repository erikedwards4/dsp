...
int gauss_s (float *Y, const size_t L, const float a, const size_t norm)
{
    ...
    const float p = -2.0f*(a*a/((L-1u)*(L-1u)));
    const float m = 0.5f*(L-1u);

    for (size_t l=0u; l<L/2u; ++l, ++Y) { *Y = expf((l-m)*(l-m)*p); }
    if (L%2u) { *Y++ = 1.0f; }
...
int gauss_d (double *Y, const size_t L, const double a, const size_t norm)
{
    ...
    const double p = -2.0*(a*a/((L-1u)*(L-1u)));
    const double m = 0.5*(L-1u);
    
    for (size_t l=0u; l<L/2u; ++l, ++Y) { *Y = exp((l-m)*(l-m)*p); }
    if (L%2u) { *Y++ = 1.0; }
    for (size_t l=0u; l<L/2u; ++l, ++Y) { *Y = *(Y-(2u*l+1u+L%2u)); }
...
int gauss_c (float *Y, const size_t L, const float a, const size_t norm)
{
    ...
    for (size_t l=0u; l<L/2u; ++l, ++Y) { *Y = expf((l-m)*(l-m)*p); *++Y = 0.0f; }
...
int gauss_z (double *Y, const size_t L, const double a, const size_t norm)
{
    ...
    for (size_t l=0u; l<L/2u; ++l, ++Y) { *Y = exp((l-m)*(l-m)*p); *++Y = 0.0; }
    if (L%2u) { *Y++ = 1.0; *Y++ = 0.0; }
