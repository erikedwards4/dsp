    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    for (size_t b=B; b>0u; --b, X-=2u*K*Lx-1u, Y1-=2u*nfft, Y-=2u*K*nfft-2u)
                    {
                        for (size_t l=Lx; l>0u; --l, X+=2u*K-1u, ++X1) { *X1 = *X; *++X1 = *++X; }
                        X1 -= 2u*Lx;
                        fftwf_execute(plan);
                        for (size_t n=nfft; n>0u; --n, ++Y1, Y+=2u*K-1u) { *Y = *Y1; *++Y = *++Y1; }
                    }
                }
                Y -= 2u*G*B*nfft;
            }
