...
//1. for (size_t l=Lw; l>0u; --l, ++Z, ++X2) { sm += (float)*Z * *X2; }
//2. for (size_t l=Lw; l>0u; --l, ++Z, ++X2) { if (*Z) { sm += *X2; } }
    ...
    else
    {
        ...
        if (K==1u && (G==1u || B==1u))
        {
            ...
            for (size_t v=V; v>0u; --v)
            {
                ...
                {
                    sp = (*X1++<0.0f); *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, ++X1, ++Z) { s = (*X1<0.0f); *Z = (s!=sp); sp = s; }
                }
                else if (going==1)
                {
                    sp = (*X1++>=0.0f); *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, ++X1, ++Z) { s = (*X1>=0.0f); *Z = s*(s!=sp); sp = s; }
                }
                else if (going==-1)
                {
                    sp = (*X1++<0.0f); *Z++ = 0;
                ...
                {
                    sm = 0.0f;
                    //for (size_t l=Lw; l>0u; --l, ++Z, ++X2) { sm = fmaf((float)*Z,*X2,sm); }
        ...
        else
        {
            for (size_t g=G; g>0u; --g, X1+=B*(Lx-1u), Y+=B*(W-1u))
            {
                for (size_t b=B; b>0u; --b, X1-=K*Lx-1u, Y-=K*W-1u)
                {
                    ...
                    {
                        sp = (*X1<0.0f); X1+=K; *Z++ = 0;
                        for (size_t l=Lx; l>1u; --l, X1+=K, ++Z) { s = (*X1<0.0f); *Z = (s!=sp); sp = s; }
                    }
                    else if (going==1)
                    {
                        sp = (*X1>=0.0f); X1+=K; *Z++ = 0;
                        for (size_t l=Lx; l>1u; --l, X1+=K, ++Z) { s = (*X1>=0.0f); *Z = s*(s!=sp); sp = s; }
                    }
                    else if (going==-1)
                    {
                        sp = (*X1<0.0f); X1+=K; *Z++ = 0;
    ...
    else
    {
        ...
        if (K==1u && (G==1u || B==1u))
        {
            ...
            for (size_t v=V; v>0u; --v)
            {
                ...
                {
                    sp = (*X1++<0.0); *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, ++X1, ++Z) { s = (*X1<0.0); *Z = (s!=sp); sp = s; }
                }
                else if (going==1)
                {
                    sp = (*X1++>=0.0); *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, ++X1, ++Z) { s = (*X1>=0.0); *Z = s*(s!=sp); sp = s; }
                }
                else if (going==-1)
                {
                    sp = (*X1++<0.0); *Z++ = 0;
        ...
        else
        {
            for (size_t g=G; g>0u; --g, X1+=B*(Lx-1u), Y+=B*(W-1u))
            {
                for (size_t b=B; b>0u; --b, X1-=K*Lx-1u, Y-=K*W-1u)
                {
                    ...
                    {
                        sp = (*X1<0.0); X1+=K; *Z++ = 0;
                        for (size_t l=Lx; l>1u; --l, X1+=K, ++Z) { s = (*X1<0.0); *Z = (s!=sp); sp = s; }
                    }
                    else if (going==1)
                    {
                        sp = (*X1>=0.0); X1+=K; *Z++ = 0;
                        for (size_t l=Lx; l>1u; --l, X1+=K, ++Z) { s = (*X1>=0.0); *Z = s*(s!=sp); sp = s; }
                    }
                    else if (going==-1)
                    {
                        sp = (*X1<0.0); X1+=K; *Z++ = 0;
...
int zcr_windowed_c (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t Lw, const size_t W, const size_t dim, const float c0, const float stp, const int going)
{
    ...
    else
    {
        ...
        for (size_t g=G; g>0u; --g, X1+=2u*B*(Lx-1u), Y+=B*(W-1u))
        {
            for (size_t b=B; b>0u; --b, X1-=2u*K*Lx-2u, Y-=K*W-1u)
            {
                //Get Z
                if (going==0)
                {
                    sp = (*X1<0.0f); X1+=2u*K; *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, X1+=2u*K, ++Z) { s = (*X1<0.0f); *Z = (s!=sp); sp = s; }
                }
                else if (going==1)
                {
                    sp = (*X1>=0.0f); X1+=2u*K; *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, X1+=2u*K, ++Z) { s = (*X1>=0.0f); *Z = s*(s!=sp); sp = s; }
                }
                else if (going==-1)
                {
                    sp = (*X1<0.0f); X1+=2u*K; *Z++ = 0;
        ...
        for (size_t g=G; g>0u; --g, X1+=2u*B*(Lx-1u), Y+=B*(W-1u))
        {
            for (size_t b=B; b>0u; --b, X1-=2u*K*Lx-2u, Y-=K*W-1u)
            {
                ...
                if (going==0)
                {
                    ...
                    for (size_t l=Lx; l>1u; --l, X1+=2u*K, ++Z) { s = (*X1<0.0); *Z = (s!=sp); sp = s; }
                }
                else if (going==1)
                {
                    sp = (*X1>=0.0); X1+=2u*K; *Z++ = 0;
                    for (size_t l=Lx; l>1u; --l, X1+=2u*K, ++Z) { s = (*X1>=0.0); *Z = s*(s!=sp); sp = s; }
                }
                else if (going==-1)
                {
                    sp = (*X1<0.0); X1+=2u*K; *Z++ = 0;
