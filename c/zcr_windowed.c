...
int zcr_windowed_s (float *Y, const float *X, const char iscolmajor, const int R, const int C, const float *W, const int L, const int dim, const int c0, const float stp, const int going)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in zcr_windowed_s: R (nrows X) must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in zcr_windowed_s: C (ncols X) must be positive\n"); return 1; }
    if (L<1) { fprintf(stderr,"error in zcr_windowed_s: L (winlength) must be positive\n"); return 1; }

    //Initialize Z
    if (!(Z=(char *)malloc((size_t)(N)*sizeof(char)))) { fprintf(stderr,"error in zcr_windowed_s: problem with malloc. "); perror("malloc"); return 1; }

    //Initialize W2 (same as W if L even, else 0.5*W added to itself with lag 1)
    if (!(W2=(float *)malloc((size_t)(L)*sizeof(float)))) { fprintf(stderr,"error in zcr_windowed_s: problem with malloc. "); perror("malloc"); return 1; }
    ...
    {
        fprintf(stderr,"error in zcr_windowed_s: dim must be 0 or 1.\n"); return 1;
    }
