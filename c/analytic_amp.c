...
int analytic_amp_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t nfft)
{
    if (dim>3u) { fprintf(stderr,"error in analytic_amp_s: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (nfft<L) { fprintf(stderr,"error in analytic_amp_s: nfft must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!fplan) { fprintf(stderr,"error in analytic_amp_s: problem creating fftw plan"); return 1; }
        fftwf_plan iplan = fftwf_plan_dft_1d((int)nfft,(fftwf_complex *)Y1,(fftwf_complex *)Z1,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in analytic_amp_s: problem creating fftw plan"); return 1; }
...
int analytic_amp_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const size_t nfft)
{
    if (dim>3u) { fprintf(stderr,"error in analytic_amp_d: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (nfft<L) { fprintf(stderr,"error in analytic_amp_d: nfft must be >= L (vec length)\n"); return 1; }
    ...
    else
    {
        ...
        if (!fplan) { fprintf(stderr,"error in analytic_amp_d: problem creating fftw plan"); return 1; }
        fftw_plan iplan = fftw_plan_dft_1d((int)nfft,(fftw_complex *)Y1,(fftw_complex *)Z1,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!iplan) { fprintf(stderr,"error in analytic_amp_d: problem creating fftw plan"); return 1; }
