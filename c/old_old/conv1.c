...
//This is identical to xcorr1, except X2 is in flipped order.

//The framing is controlled by inputs str, es0, Ly (stride, 1st start sample, length of vecs in Y).

//This is like conv1d in allowing stride (str), but doesn't allow padding and dilation.
...
int conv1_s (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim);
int conv1_d (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim);
int conv1_c (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim);
int conv1_z (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim);


int conv1_s (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in conv1_s: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in conv1_s: str (stride) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv1_s: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in conv1_s: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in conv1_s: L1 (length of vecs in X1) must be positive\n"); return 1; }
...
int conv1_d (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in conv1_d: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in conv1_d: str (stride) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv1_d: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in conv1_d: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in conv1_d: L1 (length of vecs in X1) must be positive\n"); return 1; }
...
int conv1_c (float *Y, const float *X1, const float *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in conv1_c: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in conv1_c: str (stride) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv1_c: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in conv1_c: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in conv1_c: L1 (length of vecs in X1) must be positive\n"); return 1; }
...
int conv1_z (double *Y, const double *X1, const double *X2, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t L2, const int es0, const size_t str, const size_t Ly, const size_t dim)
{
    if (dim>3u) { fprintf(stderr,"error in conv1_z: dim must be in [0 3]\n"); return 1; }
    if (str<1u) { fprintf(stderr,"error in conv1_z: str (stride) must be positive\n"); return 1; }
    if (L2<1u) { fprintf(stderr,"error in conv1_z: L2 (length of X2) must be positive\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L1 = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (N<1u) { fprintf(stderr,"error in conv1_z: N (total length of X1) must be positive\n"); return 1; }
    if (L1<1u) { fprintf(stderr,"error in conv1_z: L1 (length of vecs in X1) must be positive\n"); return 1; }
