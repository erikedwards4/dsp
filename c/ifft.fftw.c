...
//Y is real-valued for iff_fftw_s and iff_fftw_d,
//and complex-valued for iff_fftw_c and iff_fftw_z.
...
int iff_fftw_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc);
int iff_fftw_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc);
int iff_fftw_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc);
int iff_fftw_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc);


int iff_fftw_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc)
{
    if (dim>3u) { fprintf(stderr,"error in iff_fftw_s: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    const float s = (sc) ? (float)(2.0*M_SQRT2/nfft) : 1.0f/nfft;
    if (Lx!=nfft/2u+1u) { fprintf(stderr,"error in iff_fftw_s: nfrqs (vec length in X) must equal nfft/2+1\n"); return 1; }
    ...
    else
    {
        ...
        if (!plan) { fprintf(stderr,"error in iff_fftw_s: problem creating fftw plan"); return 1; }
...
int iff_fftw_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc)
{
    if (dim>3u) { fprintf(stderr,"error in iff_fftw_d: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    const double s = (sc) ? 2.0*M_SQRT2/nfft : 1.0f/nfft;
    if (Lx!=nfft/2u+1u) { fprintf(stderr,"error in iff_fftw_d: nfrqs (vec length in X) must equal nfft/2+1\n"); return 1; }
    ...
    else
    {
        ...
        if (!plan) { fprintf(stderr,"error in iff_fftw_d: problem creating fftw plan"); return 1; }
...
int iff_fftw_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc)
{
    if (dim>3u) { fprintf(stderr,"error in iff_fftw_c: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    const float s = (sc) ? (float)(2.0*M_SQRT2/nfft) : 1.0f/nfft;
    if (Lx!=nfft) { fprintf(stderr,"error in iff_fftw_c: nfrqs (vec length in X) must equal nfft\n"); return 1; }
    ...
    else
    {
        ...
        Y1 = (float *)fftwf_malloc(2u*nfft*sizeof(float));
        fftwf_plan plan = fftwf_plan_dft_1d((int)nfft,(fftwf_complex *)X1,(fftwf_complex *)Y1,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!plan) { fprintf(stderr,"error in iff_fftw_c: problem creating fftw plan"); return 1; }
...
int iff_fftw_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const size_t nfft, const char sc)
{
    if (dim>3u) { fprintf(stderr,"error in iff_fftw_z: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    const float s = (sc) ? 2.0*M_SQRT2/nfft : 1.0f/nfft;
    if (Lx!=nfft) { fprintf(stderr,"error in iff_fftw_z: nfrqs (vec length in X) must equal nfft\n"); return 1; }
    ...
    else
    {
        ...
        Y1 = (double *)fftw_malloc(2u*nfft*sizeof(double));
        fftw_plan plan = fftw_plan_dft_1d((int)nfft,(fftw_complex *)X1,(fftw_complex *)Y1,FFTW_BACKWARD,FFTW_ESTIMATE);
        if (!plan) { fprintf(stderr,"error in iff_fftw_z: problem creating fftw plan"); return 1; }
