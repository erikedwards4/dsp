...
int ac2ar_levdurb_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else
    {
        ...
        float *A, *Atmp, a, e;
        if (!(A=(float *)malloc(P*sizeof(float)))) { fprintf(stderr,"error in ac2ar_levdurb_s: problem with malloc. "); perror("malloc"); return 1; }
        if (!(Atmp=(float *)malloc((P-1u)*sizeof(float)))) { fprintf(stderr,"error in ac2ar_levdurb_s: problem with malloc. "); perror("malloc"); return 1; }
        
        struct timespec tic, toc; clock_gettime(CLOCK_REALTIME,&tic);

        if (Lx==N)
        {
            // Y[0] = AStmp[0] = a = -X1[1]/X1[0];
            // e = fmaf(X1[1],a,X1[0]);
            // for (p=1; p<P; p++)
            // {
            //     g = X1[p+1];
            //     for (q=0; q<p; q++) { g = fmaf(X1[q+1],Y[p-q-1],g); }
            //     Y[p] = g = -g/e;
            //     for (q=0; q<p; q++) { Y[q] = fmaf(g,AStmp[p-q-1],Y[q]); }
            //     cblas_scopy(p,&Y[0],1,&AStmp[0],1);
            //     e *= fmaf(g,-g,1.0f);
            // }
            ...
            *Y++ = *Atmp++ = a;
            e = *X++; e += a * *X++;
            // for (size_t p=1u; p<P; ++p, ++X, ++Y)
            // {
            //     a = *X; X -= p;
            //     for (size_t q=0u; q<p; ++q, ++X) { --Y; a += *X * *Y; }
            //     a /= -e; *(Y+p) = a;
            //     for (size_t q=0u; q<p; ++q, ++Y) { --Atmp; *Y += a * *Atmp; }
            //     Y -= p;
            //     for (size_t q=0u; q<p; ++q, ++Y, ++Atmp) { *Atmp = *Y; }
            //     e *= 1.0f - a*a;
            // }
            for (size_t p=2u; p<P; ++p, ++X, ++Y)
            {
                a = *X;
                X -= p - 1u;
                for (size_t q=1u; q<p; ++q, ++X) { --Y; a += *X * *Y; }
                a /= -e; *(Y+p-1u) = a;
                for (size_t q=1u; q<p; ++q, ++Y) { --Atmp; *Y += a * *Atmp; }
                Y -= p - 1u;
                for (size_t q=0u; q<p; ++q, ++Y, ++Atmp) { *Atmp = *Y; }
                e *= 1.0f - a*a;
            }
...
int ac2ar_levdurb_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else
    {
        const size_t P = Lx - 1u;
        double *A, *Atmp, a, e;
        if (!(A=(double *)malloc(P*sizeof(double)))) { fprintf(stderr,"error in ac2ar_levdurb_d: problem with malloc. "); perror("malloc"); return 1; }
        if (!(Atmp=(double *)malloc((P-1u)*sizeof(double)))) { fprintf(stderr,"error in ac2ar_levdurb_d: problem with malloc. "); perror("malloc"); return 1; }
...
int ac2ar_levdurb_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else
    {
        ...
        if (!(A=(float *)malloc(2u*P*sizeof(float)))) { fprintf(stderr,"error in ac2ar_levdurb_c: problem with malloc. "); perror("malloc"); return 1; }
        if (!(Atmp=(float *)malloc(2u*(P-1u)*sizeof(float)))) { fprintf(stderr,"error in ac2ar_levdurb_c: problem with malloc. "); perror("malloc"); return 1; }
...
int ac2ar_levdurb_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else
    {
        ...
        if (!(A=(double *)malloc(2u*P*sizeof(double)))) { fprintf(stderr,"error in ac2ar_levdurb_z: problem with malloc. "); perror("malloc"); return 1; }
        if (!(Atmp=(double *)malloc(2u*(P-1u)*sizeof(double)))) { fprintf(stderr,"error in ac2ar_levdurb_z: problem with malloc. "); perror("malloc"); return 1; }
