...
int rect_pulse_s (float *Y, const size_t N, const size_t samp, const size_t width, const float amp)
{
    ...
    if (samp+width>=N) { fprintf(stderr, "error in rect_pulse_s: samp+width must be less than N\n"); return 1; }
    ...
    for (size_t n=width; n>0u; --n, ++Y) { *Y = amp; }
    for (size_t n=N-samp-width; n>0u; --n, ++Y) { *Y = 0.0f; }
...
int rect_pulse_d (double *Y, const size_t N, const size_t samp, const size_t width, const double amp)
{
    if (samp+width>=N) { fprintf(stderr, "error in rect_pulse_d: samp+width must be less than N\n"); return 1; }

    for (size_t n=samp; n>0u; --n, ++Y) { *Y = 0.0; }
    for (size_t n=width; n>0u; --n, ++Y) { *Y = amp; }
    for (size_t n=N-samp-width; n>0u; --n, ++Y) { *Y = 0.0; }
...
int rect_pulse_c (float *Y, const size_t N, const size_t samp, const size_t width, const float amp)
{
    if (samp+width>=N) { fprintf(stderr, "error in rect_pulse_c: samp+width must be less than N\n"); return 1; }

    for (size_t n=2u*samp; n>0u; --n, ++Y) { *Y = 0.0f; }
    for (size_t n=width; n>0u; --n) { *Y++ = amp; *Y++ = 0.0f; }
    for (size_t n=2u*(N-samp-width); n>0u; --n, ++Y) { *Y = 0.0f; }
...
int rect_pulse_z (double *Y, const size_t N, const size_t samp, const size_t width, const double amp)
{
    if (samp+width>=N) { fprintf(stderr, "error in rect_pulse_z: samp+width must be less than N\n"); return 1; }

    for (size_t n=2u*samp; n>0u; --n, ++Y) { *Y = 0.0; }
    for (size_t n=width; n>0u; --n) { *Y++ = amp; *Y++ = 0.0; }
    for (size_t n=2u*(N-samp-width); n>0u; --n, ++Y) { *Y = 0.0; }
