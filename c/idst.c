    ...
    else
    {
        ...
        if (L==N)
        {
            *X1++ = *X++;
            ...
            X1 -= ndst;
            fftwf_execute(plan);
            for (size_t l=0u; l<ndst; ++l, ++Y1, ++Y) { *Y = *Y1 * ysc; }
            Y1 -= ndst;
        }
        else
        {
                ...
                for (size_t g=0u; g<G; ++g, X+=B*(L-1u), Y+=B*(ndst-1u))
                {
                    for (size_t b=0u; b<B; ++b, X-=K*L-1u, Y1-=ndst, Y-=K*ndst-1u)
                    {
                        *X1++ = *X; X += K;
                        ...
                        X1 -= L;
                        fftwf_execute(plan);
                        for (size_t l=0u; l<ndst; ++l, ++Y1, Y+=K) { *Y = *Y1 * ysc; }
                    }
                }
            }
        }
    ...
    else
    {
        ...
        if (L==N)
        {
            *X1r++ = *X++; *X1i++ = *X++;
            ...
            for (size_t l=0u; l<ndst; ++l, ++Y1r, ++Y1i, ++Y) { *Y = *Y1r*ysc; *++Y = *Y1i*ysc; }
            Y1r -= ndst; Y1i -= ndst;
        }
        else
        {
                ...
                for (size_t g=0u; g<G; ++g, X+=2u*B*(L-1u), Y+=2u*B*(ndst-1u))
                {
                    for (size_t b=0u; b<B; ++b, X-=2u*K*L-2u, Y1r-=ndst, Y1i-=ndst, Y-=2u*K*ndst-2u)
                    {
                        *X1r++ = *X; *X1i++ = *++X; X += 2u*K-1u;
                        ...
                        X1r -= L; X1i -= L;
                        fftwf_execute(rplan); fftwf_execute(iplan);
                        for (size_t l=0u; l<ndst; ++l, ++Y1r, ++Y1i, Y+=2u*K-1u) { *Y = *Y1r*ysc; *++Y = *Y1i*ysc; }
                    }
                }
            }
        }
