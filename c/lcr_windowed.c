//This gets level crossings (LCs) as usual from X1,
//and then applies window X2 to get LC rate (LCR).
//
...
{
    if (dim>3u) { fprintf(stderr,"error in lcr_windowed_s: dim must be in [0 3]\n"); return 1; }
    if (Lw<1u) { fprintf(stderr,"error in lcr_windowed_s: Lw (winlength) must be positive\n"); return 1; }
    if (stp<FLT_EPSILON) { fprintf(stderr,"error in lcr_windowed_s: stp (step size) must be positive\n"); return 1; }
    if (going!=0 && going!=1 && going!=-1) { fprintf(stderr,"error in lcr_windowed_s: going must be in {-1,0,1}\n"); return 1; }

    const size_t N = R*C*S*H;
    if (N==0u) {fprintf(stderr,"error in lcr_windowed_s: X1 is empty (size of at least one dim is 0)\n"); return 1; }

    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (Lx<1u) { fprintf(stderr,"error in lcr_windowed_s: Lx (length of vecs in X1) must be positive\n"); return 1; }
    if (Lw>Lx) { fprintf(stderr,"error in lcr_windowed_s: Lw (winlength) must be <= Lx (length of vecs in X1)\n"); return 1; }
    if (W>Lx) { fprintf(stderr,"error in lcr_windowed_s: W must be <= Lx (length of vecs in X1)\n"); return 1; }
    if (c0>(float)(Lx-1u)) { fprintf(stderr,"error in lcr_windowed_s: c0 (center samp of 1st frame) must be < Lx (length of vecs in X1)\n"); return 1; }
    ...
    else
    {
        ...
        int *Z, s, sp;                              //ints for LCR
        float sm;                                   //local sum for LCR

        //Allocate
        if (!(Z=(int *)malloc(Lx*sizeof(int)))) { fprintf(stderr,"error in lcr_windowed_s: problem with malloc. "); perror("malloc"); return 1; }
...
{
    if (dim>3u) { fprintf(stderr,"error in lcr_windowed_d: dim must be in [0 3]\n"); return 1; }
    if (Lw<1u) { fprintf(stderr,"error in lcr_windowed_d: Lw (winlength) must be positive\n"); return 1; }
    if (stp<DBL_EPSILON) { fprintf(stderr,"error in lcr_windowed_d: stp (step size) must be positive\n"); return 1; }
    if (going!=0 && going!=1 && going!=-1) { fprintf(stderr,"error in lcr_windowed_d: going must be in {-1,0,1}\n"); return 1; }

    const size_t N = R*C*S*H;
    if (N==0u) {fprintf(stderr,"error in lcr_windowed_d: X1 is empty (size of at least one dim is 0)\n"); return 1; }

    const size_t Lx = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (Lx<1u) { fprintf(stderr,"error in lcr_windowed_d: Lx (length of vecs in X1) must be positive\n"); return 1; }
    if (Lw>Lx) { fprintf(stderr,"error in lcr_windowed_d: Lw (winlength) must be <= Lx (length of vecs in X1)\n"); return 1; }
    if (W>Lx) { fprintf(stderr,"error in lcr_windowed_d: W must be <= Lx (length of vecs in X1)\n"); return 1; }
    if (c0>(double)(Lx-1u)) { fprintf(stderr,"error in lcr_windowed_d: c0 (center samp of 1st frame) must be < Lx (length of vecs in X1)\n"); return 1; }
    ...
    else
    {
        ...
        int prev_cs;                                //previous rounded center-samp
        int *Z, s, sp;                              //ints for LCR
        double sm;                                  //local sum for LCR

        //Allocate
        if (!(Z=(int *)malloc(Lx*sizeof(int)))) { fprintf(stderr,"error in lcr_windowed_d: problem with malloc. "); perror("malloc"); return 1; }
