    ...
    else
    {
        ...
        if (L==N)
        {
            *X1++ = *X++ * xsc;
            ...
            X1 -= ndct;
            fftwf_execute(plan);
            for (size_t l=0u; l<ndct; ++l, ++Y1, ++Y) { *Y = *Y1 * ysc; }
            Y1 -= ndct;
        }
        else
        {
                ...
                for (size_t g=G; g>0u; --g, X+=B*(L-1u), Y+=B*(ndct-1u))
                {
                    for (size_t b=B; b>0u; --b, X-=K*L-1u, Y1-=ndct, Y-=K*ndct-1u)
                    {
                        *X1++ = *X * xsc; X += K;
                        ...
                        X1 -= L;
                        fftwf_execute(plan);
                        for (size_t l=0u; l<ndct; ++l, ++Y1, Y+=K) { *Y = *Y1 * ysc; }
                    }
                }
            }
        }
    ...
    else
    {
        ...
        if (L==N)
        {
            *X1r++ = *X++ * xsc; *X1i++ = *X++ * xsc;
            ...
            for (size_t l=0u; l<ndct; ++l, ++Y1r, ++Y1i, ++Y) { *Y = *Y1r*ysc; *++Y = *Y1i*ysc; }
            Y1r -= ndct; Y1i -= ndct;
        }
        else
        {
                ...
                for (size_t g=G; g>0u; --g, X+=2u*B*(L-1u), Y+=2u*B*(ndct-1u))
                {
                    for (size_t b=B; b>0u; --b, X-=2u*K*L-2u, Y1r-=ndct, Y1i-=ndct, Y-=2u*K*ndct-2u)
                    {
                        *X1r++ = *X * xsc; *X1i++ = *++X * xsc; X += 2u*K-1u;
                        ...
                        X1r -= L; X1i -= L;
                        fftwf_execute(rplan); fftwf_execute(iplan);
                        for (size_t l=0u; l<ndct; ++l, ++Y1r, ++Y1i, Y+=2u*K-1u) { *Y = *Y1r*ysc; *++Y = *Y1i*ysc; }
                    }
                }
            }
        }
