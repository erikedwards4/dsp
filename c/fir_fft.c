...
int fir_fft_s (float *Y, const float *X, const float *B, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t Q, const size_t dim)
{
    ...
    else
    {
        ...
        fftwf_execute(bplan);
        *D1++ /= (float)nfft; ++D1;
        for (size_t l=nfft+isodd-2u; l>0u; --l, ++D1) { *D1 *= sc; }
        ...
        if (L==N)
        {
            ...
            *Y1 *= *D1;
            ...
            fftwf_execute(iplan);
            for (size_t l=L; l>0u; --l, Z1+=2, ++Y) { *Y = *Z1; }
            Z1 -= 2u*L;
        }
        else
        {
                ...
                for (size_t g=G; g>0u; --g, X+=BS*(L-1u), Y+=BS*(L-1u))
                {
                    for (size_t bs=BS; bs>0u; --bs, X-=K*L-1u, Y-=K*L-1u)
                    {
                        ...
                        *Y1 *= *D1;
                        D1 -= 2u*F-2u; Y1 -= 2u*F-2u;
                        fftwf_execute(iplan);
                        for (size_t l=L; l>0u; --l, Z1+=2, Y+=K) { *Y = *Z1; }
                        Z1 -= 2u*L;
                    }
                }
            }
...
int fir_fft_c (float *Y, const float *X, const float *B, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t Q, const size_t dim)
{
    ...
    else
    {
        ...
        B1 -= 2u*nfft;
        fftwf_execute(bplan);
        for (size_t l=2u*nfft; l>0u; --l, ++D1) { *D1 /= (float)nfft; }
        D1 -= 2u*nfft;
        ...
        if (L==N)
        {
            ...
            fftwf_execute(iplan);
            for (size_t l=2u*L; l>0u; --l, ++Z1, ++Y) { *Y = *Z1; }
            Z1 -= 2u*L;
        }
        else
        {
            ...
            else
            {
                for (size_t g=G; g>0u; --g, X+=2u*BS*(L-1u), Y+=2u*BS*(L-1u))
                {
                    ...
                    {
                        for (size_t l=L; l>0u; --l, X+=2u*K, ++X1) { *X1 = *X; *++X1 = *(X+1); }
                        X1 -= 2u*L;
                        fftwf_execute(xplan);
                        for (size_t n=nfft; n>0u; --n)
                        {
                        ...
                        D1 -= 2u*nfft; Y1 -= 2u*nfft;
                        fftwf_execute(iplan);
                        for (size_t l=L; l>0u; --l, ++Z1, Y+=2u*K) { *Y = *Z1; *(Y+1) = *++Z1; }
                        Z1 -= 2u*L;
                    }
                }
            }
...
int fir_fft_z (double *Y, const double *X, const double *B, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t Q, const size_t dim)
{
    ...
    else
    {
        ...
        B1 -= 2u*nfft;
        fftw_execute(bplan);
        for (size_t l=2u*nfft; l>0u; --l, ++D1) { *D1 /= (double)nfft; }
        D1 -= 2u*nfft;
