...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ squarewave -n32 -a2.5 -f0.2 -o Y \n";
    descr += "$ squarewave -n32 -f0.01 -p1.57079632679 > Y \n";
    descr += "$ squarewave -n32 -f0.01 -d1 -t1 -f101 > Y \n";
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::squarewave_s(Y,o1.N(),(float)amp,(float)frq,(float)phs))
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::squarewave_d(Y,o1.N(),(double)amp,(double)frq,(double)phs))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::squarewave_c(Y,N,(float)amp,(float)frq,(float)phs))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::squarewave_z(Y,N,(double)amp,(double)frq,(double)phs))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
