...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "This version uses the FFTS (fastest FFT in the South) library.\n";
    descr += "The DCT is done via a 4*ndct length FFT of each vec in X of length Lx.\n";
    descr += "This is slower than the Lx length DCT, but FFTS is fully open-source.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ dct.ffts -n256 X -o Y \n";
    descr += "$ dct.ffts -n256 -d1 X > Y \n";
    descr += "$ cat X | dct.ffts -n256 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::dct_ffts_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndct,sc))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::dct_ffts_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndct,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::dct_ffts_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndct,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::dct_ffts_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndct,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
