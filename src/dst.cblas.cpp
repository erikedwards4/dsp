...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "This version uses a CBLAS matrix multiplication by the DST-I matrix.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ dst.cblas -n256 X -o Y \n";
    descr += "$ dst.cblas -n256 -d1 X > Y \n";
    descr += "$ cat X | dst.cblas -n256 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::dst_cblas_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndst,sc))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::dst_cblas_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndst,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::dst_cblas_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndst,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::dst_cblas_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndst,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
