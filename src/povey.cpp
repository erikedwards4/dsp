...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets 1D Povey window (from Kaldi: \"like hamming but goes to zero at edges\").\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ povey -l255 -o Y \n";
    descr += "$ povey -l255 > Y \n";
    descr += "$ povey -l127 -d1 -t1 -f101 > Y \n";
    ...
    if (o1.T==1)
    {
        ...
        if (codee::povey_s(Y,L,norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::povey_d(Y,L,norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==101)
    {
        ...
        if (codee::povey_c(Y,L,norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==102)
    {
        ...
        if (codee::povey_z(Y,L,norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
