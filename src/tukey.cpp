...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets 1D Tukey window with cosine fraction r.\n";
    descr += "\n";
    descr += "Use -l (--winlength) to give the window length in number of taps (sample points).\n";
    descr += "\n";
    descr += "Use -r (--fraction) to give the fraction of the window length with cosine parts [default=0.5].\n";
    descr += "The fraction must be in [0 1], where 0 gives rectangular win and 1 gives Hann win.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ tukey -l255 -r0.6 -o Y \n";
    descr += "$ tukey -l255 -r0.6 > Y \n";
    descr += "$ tukey -l127 -r0.6 -d1 -t1 -f101 > Y \n";


    //Argtable
    int nerrs;
    struct arg_int   *a_wl = arg_intn("l","winlength","<uint>",0,1,"window length [default=7]");
    struct arg_dbl   *a_fr = arg_dbln("r","fraction","<dbl>",0,1,"cosine fraction [default=0.5]");
    ...
    void *argtable[] = {a_wl, a_fr, a_nrm, a_d, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    //Get r
    if (a_fr->count==0) { r = 0.5; }
    else if (a_fr->dval[0]<0.0 || a_fr->dval[0]>1.0) { cerr << progstr+": " << __LINE__ << errstr << "cosine fraction (r) must be in [0 1]" << endl; return 1; }
    else { r = a_fr->dval[0]; }
    ...
    if (o1.T==1)
    {
        ...
        if (codee::tukey_s(Y,L,float(r),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::tukey_d(Y,L,double(r),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==101)
    {
        ...
        if (codee::tukey_c(Y,L,float(r),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==102)
    {
        ...
        if (codee::tukey_z(Y,L,double(r),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
