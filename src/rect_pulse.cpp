...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Use -w (--width) to give the sample number of the impulse [default=0].\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ rect_pulse -n32 -w3 -o Y \n";
    descr += "$ rect_pulse -n32 -s9 > Y \n";
    descr += "$ rect_pulse -n32 -s1 -w5 -d1 -t1 -f101 > Y \n";


    //Argtable
    int nerrs;
    struct arg_dbl  *a_amp = arg_dbln("a","amp","<dbl>",0,1,"amplitude [default=1.0]");
    struct arg_int    *a_s = arg_intn("s","samp","<uint>",0,1,"sample number of pulse start [default=0]");
    struct arg_int    *a_w = arg_intn("w","width","<uint>",0,1,"width of pulse in samples [default=1]");
    ...
    void *argtable[] = {a_amp, a_s, a_w, a_n, a_d, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    //Get width
    if (a_w->count==0) { w = 1u; }
    else if (a_w->ival[0]<0) { cerr << progstr+": " << __LINE__ << errstr << "width must be a nonnegative int" << endl; return 1; }
    else { w = size_t(a_w->ival[0]); }
    if (w>=N) { cerr << progstr+": " << __LINE__ << errstr << "w must be < N" << endl; return 1; }
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::rect_pulse_s(Y,o1.N(),samp,w,(float)amp))
    ...
    else if (o1.T==2u)
    {
        ...
        if (codee::rect_pulse_d(Y,o1.N(),samp,w,(double)amp))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::rect_pulse_c(Y,o1.N(),samp,w,(float)amp))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::rect_pulse_z(Y,o1.N(),samp,w,(double)amp))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
