...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets the zero crossing rate (MCR) of X along dim.\n";
    ...
    descr += "Y is in units of MCs/sample (so multiply by fs to get MCs/sec).\n";
    descr += "Thus, each element of Y is in [0.0 1.0].\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ mcr -l100 X -o Y \n";
    descr += "$ mcr -d1 -l7 X > Y \n";
    descr += "$ cat X | mcr -l127 -g1 -c > Y \n";
    ...
    struct arg_int    *a_l = arg_intn("l","Lw","<uint>",0,1,"winlength over which to average MCs [default=1]");
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::mcr_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,Lw,g,causal))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::mcr_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,Lw,g,causal))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
