...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets analytic amplitude (AA) for each vector (1D signal) in X.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ analytic_amp -n256 X -o Y \n";
    descr += "$ analytic_amp -n256 -d1 X > Y \n";
    descr += "$ cat X | analytic_amp -n256 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::analytic_amp_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::analytic_amp_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
