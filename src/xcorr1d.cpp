...
int main(int argc, char *argv[])
{
    ...
    descr += "$ cat X2 | xcorr1d -d1 -p5 -s3 -i2 X1 > Y \n";
    ...
    if (int(L1)+2*pad<=int(dil*(i2.N()-1u))) { cerr << progstr+": " << __LINE__ << errstr << "L1+2*pad must be > dil*(L2-1)" << endl; return 1; }


    //Set output header info
    Ly = 1u + (L1 + 2*pad - dil*(i2.N()-1u) - 1u) / str;
    o1.F = i1.F; o1.T = i1.T;
    o1.R = (dim==0u) ? Ly : i1.R;
    o1.C = (dim==1u) ? Ly : i1.C;
    o1.S = (dim==2u) ? Ly : i1.S;
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::xcorr1d_d(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),pad,str,dil,dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::xcorr1d_z(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),pad,str,dil,dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
