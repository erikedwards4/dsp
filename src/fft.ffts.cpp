...
int main(int argc, char *argv[])
{
    ...
    descr += "but increased by 1 if L is odd (only even nfft supported here).\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ fft.ffts -n256 X -o Y \n";
    descr += "$ fft.ffts -n256 -d1 X > Y \n";
    descr += "$ cat X | fft.ffts -n256 > Y \n";
    ...
    if (nfft%2u) { cerr << progstr+": " << __LINE__ << errstr << "nfft must be even" << endl; return 1; }
    ...
    if (i1.T==2u || i1.T==102u) { cerr << progstr+": " << __LINE__ << warstr << "double precision not directly supported (casting to float and back)" << endl; }
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::fft_ffts_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::fft_ffts_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::fft_ffts_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::fft_ffts_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
