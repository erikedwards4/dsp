...
int main(int argc, char *argv[])
{
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::violet_s(Y,o1.N(),(float)std,zmn))
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::violet_d(Y,o1.N(),(double)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::violet_c(Y,o1.N(),(float)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::violet_z(Y,o1.N(),(double)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
