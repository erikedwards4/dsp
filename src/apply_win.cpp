...
int main(int argc, char *argv[])
{
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::apply_win_inplace_d(X1,X2,i1.R,i1.C,i1.S,i1.H,i2.N(),dim,o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X1),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::apply_win_inplace_z(X1,X2,i1.R,i1.C,i1.S,i1.H,i2.N(),dim,o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X1),o1.nbytes()); }
