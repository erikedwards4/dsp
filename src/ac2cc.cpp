...
int main(int argc, char *argv[])
{
    ...
    descr += "The linear prediction (LP) coeffs are used to compute the CCs in Y.\n";
    descr += "\n";
    ...
    struct arg_int    *a_k = arg_intn("k","K","<uint>",0,1,"number of CCs to compute [default=Lx-1]");
    ...
    if (K>=Lx) { cerr << progstr+": " << __LINE__ << errstr << "K must be < Lx (length of vecs in X)" << endl; return 1; }
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::ac2cc_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,K,float(preg))) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::ac2cc_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,K,double(preg))) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
