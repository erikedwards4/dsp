...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Use -w (--nframes) to give W, the number of frames [default=N/stp].\n";
    descr += "This is a positive int (use less than N/stp to use only part of X).\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ frame_univar_float -l255 -s65 -c0 X -o Y \n";
    descr += "$ frame_univar_float -l255 -w20 -c60.25 -s60.25 X > Y \n";
    descr += "$ cat X | frame_univar_float -l127 > Y \n";
    ...
    struct arg_int    *a_w = arg_intn("w","nframes","<uint>",0,1,"number of frames [default=N/stp]");
    struct arg_file  *a_fo = arg_filen("o","ofile","<file>",0,O,"output file (Y)");
    struct arg_lit *a_help = arg_litn("h","help",0,1,"display this help and exit");
    struct arg_end  *a_end = arg_end(5);
    void *argtable[] = {a_fi, a_c0, a_stp, a_wl, a_w, a_fo, a_help, a_end};
    ...
    //Get W
    if (a_w->count==0) { W = 1u + size_t((double(i1.N()-1u)-c0)/stp); }
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::frame_univar_float_s(Y,X,i1.N(),W,L,float(c0),float(stp)))
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::frame_univar_double_d(Y,X,i1.N(),W,L,double(c0),double(stp)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::frame_univar_float_c(Y,X,i1.N(),W,L,float(c0),float(stp)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::frame_univar_double_z(Y,X,i1.N(),W,L,double(c0),double(stp)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
