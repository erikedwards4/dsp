...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ ifft.kiss X -o Y \n";
    descr += "$ ifft.kiss -d1 -r X > Y \n";
    descr += "$ cat X | ifft.kiss -n9 -r > Y \n";
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::ifft_kiss_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
    ...
    else if (o1.T==2u)
    {
        ...
        if (codee::ifft_kiss_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::ifft_kiss_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::ifft_kiss_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft,sc)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
