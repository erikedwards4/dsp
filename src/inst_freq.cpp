...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ inst_freq -n256 X -o Y \n";
    descr += "$ inst_freq -n256 -d1 X > Y \n";
    descr += "$ cat X | inst_freq -n256 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::inst_freq_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::inst_freq_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
