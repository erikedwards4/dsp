...
int main(int argc, char *argv[])
{
    ...
    descr += "This uses Levinson-Durbin recursion on each AC function in X.\n";
    descr += "\n";
    ...
    descr += "where P = L-1 is the order of the polynomial,\n";
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::ac2psd_d(Y,X,W,i2.N(),i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::ac2psd_z(Y,X,W,i2.N(),i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
