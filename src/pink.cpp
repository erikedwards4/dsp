...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Zero-mean, Gaussian pink noise (1-D).\n";
    descr += "Makes vector of Gaussian white noise with specified stddev, and then\n";
    descr += "takes FFT, multiplies by 1/f characteristic, and takes IFFT.\n";
    descr += "\n";
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::pink_s(Y,o1.N(),(float)std,zmn))
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::pink_d(Y,o1.N(),(double)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::pink_c(Y,o1.N(),(float)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::pink_z(Y,o1.N(),(double)std,zmn))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(&Y[0]),o1.nbytes()); }
