...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_is, a_os, a_fo, a_help, a_end};
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::convert_freqs_d(X,i1.N(),iscale.c_str(),oscale.c_str())) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
