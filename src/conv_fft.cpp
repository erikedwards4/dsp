...
int main(int argc, char *argv[])
{
    ...
    descr += "but uses the FFT internally for computation.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ conv_fft X1 X2 -o Y \n";
    descr += "$ conv_fft -d1 -s'same' X1 X2 > Y \n";
    descr += "$ cat X2 | conv_fft -d1 -s'valid' X1 - > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::conv_fft_s(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),shape.c_str(),dim))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::conv_fft_d(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),shape.c_str(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::conv_fft_c(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),shape.c_str(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::conv_fft_z(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),shape.c_str(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
