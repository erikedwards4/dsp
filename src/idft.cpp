...
int main(int argc, char *argv[])
{
    ...
    descr += "This version uses a direct matrix multiplication by the IDFT matrix.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ idft -n16 X -o Y \n";
    descr += "$ idft -d1 -n16 -r X > Y \n";
    descr += "$ cat X | idft -d1 -n40 -r > Y \n";
    ...
    if (o1.T==1u)
    {
        ...
        if (codee::idft_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
    ...
    else if (o1.T==2u)
    {
        ...
        if (codee::idft_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101u)
    {
        ...
        if (codee::idft_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==102u)
    {
        ...
        if (codee::idft_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,ndft,sc))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
