...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets 1D Planck-taper window with taper fraction parameter epsilon.\n";
    descr += "\n";
    descr += "Use -l (--winlength) to give the window length in number of taps (sample points).\n";
    descr += "\n";
    descr += "Use -e (--epsilon) to give twice the fraction of the window length with taper [default=0.25].\n";
    descr += "The fraction must be in [0 0.5], where 0 gives rectangular win and 0.5 has no flat top.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ planck -l255 -r0.6 -o Y \n";
    descr += "$ planck -l255 -r0.6 > Y \n";
    descr += "$ planck -l127 -r0.6 -d1 -t1 -f101 > Y \n";


    //Argtable
    int nerrs;
    struct arg_int   *a_wl = arg_intn("l","winlength","<uint>",0,1,"window length [default=7]");
    struct arg_dbl  *a_eps = arg_dbln("e","epsilon","<dbl>",0,1,"twice the taper fraction [default=0.25]");
    ...
    void *argtable[] = {a_wl, a_eps, a_nrm, a_d, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    //Get epsilon
    if (a_eps->count==0) { epsilon = 0.25; }
    else if (a_eps->dval[0]<0.0 || a_eps->dval[0]>0.5) { cerr << progstr+": " << __LINE__ << errstr << "epsilon must be in [0 0.5]" << endl; return 1; }
    else { epsilon = a_eps->dval[0]; }
    ...
    if (o1.T==1)
    {
        ...
        if (codee::planck_s(Y,L,float(epsilon),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::planck_d(Y,L,double(epsilon),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==101)
    {
        ...
        if (codee::planck_c(Y,L,float(epsilon),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (o1.T==102)
    {
        ...
        if (codee::planck_z(Y,L,double(epsilon),norm)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
