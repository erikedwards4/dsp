...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets instantaneous phase (IPh) for each vector (1D signal) in X.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ inst_phase -n256 X -o Y \n";
    descr += "$ inst_phase -n256 -d1 X > Y \n";
    descr += "$ cat X | inst_phase -n256 > Y \n";
    ...
    void *argtable[] = {a_fi, a_d, a_n, a_fo, a_help, a_end};
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::inst_phase_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::inst_phase_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,nfft))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
