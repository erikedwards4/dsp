...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_lvl, a_c0, a_stp, a_w, a_d, a_g, a_fo, a_help, a_end};
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::lcr_windowed_d(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),i2.N(),W,dim,double(c0),double(stp),g,double(lvl)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
